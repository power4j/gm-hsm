...
use gm_hsm_sys::sgd::types::{BOOL, BYTE, CHAR, FALSE, HANDLE, LPSTR, TRUE, ULONG};
use gm_hsm_sys::skf::error::{SAR_OK, SAR_PIN_INCORRECT};
use gm_hsm_sys::skf::types::{
    ECCCipherBlob, ECCPublicKeyBlob, ECCSignatureBlob, EnvelopedKeyBlob, FileAttribute,
};
...
impl From<&FileAttribute> for FileAttr {
    fn from(value: &FileAttribute) -> Self {
        let file_name: String = unsafe {
            parse_cstr_lossy(value.file_name.as_ptr(), value.file_name.len())
                .unwrap_or("".to_string())
        };
        FileAttr {
            file_name,
