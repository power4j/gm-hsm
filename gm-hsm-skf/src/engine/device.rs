...
use crate::{
    ...
    ManagedKey, SkfApp, SkfBlockCipher, SkfDevice, Version,
};
use crate::{Error, Result};
use gm_hsm_sys::sgd::types::{BYTE, CHAR, DWORD, HANDLE, LPSTR, ULONG};
use gm_hsm_sys::skf::error::SAR_OK;
use gm_hsm_sys::skf::types::{
    DeviceInfo, ECCCipherBlob, ECCPrivateKeyBlob, ECCPublicKeyBlob, ECCSignatureBlob,
    DEV_LOCK_FOREVER,
};
use gm_hsm_sys::utils::mem::{parse_cstr_list_lossy, parse_cstr_lossy};
use std::fmt::Debug;
use std::sync::Arc;
...
impl AppManager for SkfDeviceImpl {
    ...
    fn enumerate_app_name(&self) -> Result<Vec<String>> {
        ...
        let list = unsafe { parse_cstr_list_lossy(buff.as_ptr(), buff.len()) };
        Ok(list)
    }
...
impl From<&DeviceInfo> for DeviceInformation {
    fn from(value: &DeviceInfo) -> Self {
        ...
        let manufacturer: String = unsafe {
            parse_cstr_lossy(value.manufacturer.as_ptr(), value.manufacturer.len())
                .unwrap_or("".to_string())
        };
        let issuer: String = unsafe {
            parse_cstr_lossy(value.issuer.as_ptr(), value.issuer.len()).unwrap_or("".to_string())
        };
        let label: String = unsafe {
            parse_cstr_lossy(value.label.as_ptr(), value.label.len()).unwrap_or("".to_string())
        };
        let serial_number = unsafe {
            parse_cstr_lossy(value.serial_number.as_ptr(), value.serial_number.len())
                .unwrap_or("".to_string())
        };
